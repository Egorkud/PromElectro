...
class Nasekomym_net(BaseParser):
    def __init__(self):
        super().__init__()


    def scrap(self, filename: str = "New_nasekomym_net_parsed(photos).xlsx",
              export_file: str = "nasekomym_net_parse.xlsx"):
        ...
        for row in range(2, work_sheet.max_row + 1):
            ...
            ru_link = searched_item_link.replace("nasekomym.net/uk/", "nasekomym.net/")  # Old link to new UKR link
    ...
    def get_characteristics(self, soup, row):
        ...
        try:
            characteristics = (soup.find("table", class_="table table-bordered")
                               .find_all("tr"))  # Get characteristics

                # Get keys values (can be some for find_all loops)
                # Implement scrap
            for tr in characteristics:
                char_data = tr.find_all("td")
                key, value = char_data[0].text, char_data[1].text
    ...
    def get_descriptions(self, two_lang_links, row):
        ...
        for idx, language_link in enumerate(two_lang_links):
            try:
                ...
                description_lines = soup_descr.find("div", class_="more-about").contents
    ...
    def get_photos(self, soup, row):
        ...
        try:
            # Implement scrapping
            photo_data = (soup.find("div", class_="image-additional")
                          .find_all("a"))

            photo_links = [f"https://nasekomym.net/{link.get("href")}" for link in photo_data]
    ...
    def get_product_name(self, two_lang_links, row, item_articule, work_sheet):
        ...
        for idx, language_link in enumerate(two_lang_links):
            try:
                ...
                manufacturers = ("Verk Group ", ...)  # Manufacturers names with 1 space after
                ...
                # Implement scrapping
                full_name = soup_name.find("div", class_="col-sm-6 product-info").find("h1").text
                ...
                new_name = f"{full_name} {item_articule}"
                print(new_name)

                self.blank_sheet.cell(row, 4 + idx).value = new_name

                # Save names data
                self.save_names_data("nasekomym_net", None, None, item_articule, series, None, row, idx)
